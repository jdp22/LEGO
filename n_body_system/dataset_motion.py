...
class MotionDataset():
    ...
    def __init__(self, partition, max_samples, delta_frame=10, case='run'):
        if case == 'walk':
...
class MotionDynamicsDataset():

    def __init__(self, partition, max_samples,num_timesteps=5, delta_frame=10, case='run'):
        self.num_timesteps = num_timesteps
        if case == 'walk':
            with open(os.path.join('/home/jiangdapeng/egnn/n_body_system/dataset', 'motion.pkl'), 'rb') as f:
                edges, X = pkl.load(f)
        elif case == 'run':
            with open(os.path.join('/home/jiangdapeng/egnn/n_body_system/dataset', 'motion_run.pkl'), 'rb') as f:
                edges, X = pkl.load(f)
        else:
            raise RuntimeError('Unknown case')
        ...
        if case == 'walk':
            ...
            val_case_id = [3, 8, 11, 12, 15, 18]
            test_case_id = [6, 19, 21, 0, 22, 10]
            split_dir = os.path.join('/home/jiangdapeng/egnn/n_body_system/dataset', 'split.pkl')
        elif case == 'run':
            train_case_id = [1, 2, 5, 6, 10]
            val_case_id = [0, 4, 9]
            test_case_id = [3, 7, 8]
            split_dir = os.path.join('/home/jiangdapeng/egnn/n_body_system/dataset', 'split_run.pkl')
        else:
            raise RuntimeError('Unknown case')
        ...
        for i in mapping:
            ...
            cur_x_0 = X[i][st]
            cur_v_0 = V[i][st]
            cur_x_t = [X[i][st + ii//self.num_timesteps*delta_frame] for ii in range(1, self.num_timesteps + 1)]
            cur_x_t = np.stack(cur_x_t,axis=-2)
            cur_v_t = [V[i][st + ii//self.num_timesteps*delta_frame] for ii in range(1, self.num_timesteps + 1)]
    ...
    def __getitem__(self, i):
        ...
        # return self.x_0[i], self.v_0[i], edges, edge_attr, local_edges, local_edge_attr, \
        #        node_fea, self.x_t[i]
        return self.x_0[i], self.v_0[i],edge_attr,\
               node_fea, self.x_t[i]
...
if __name__ == '__main__':
    data = MotionDynamicsDataset(partition='train', max_samples=500, delta_frame=10)