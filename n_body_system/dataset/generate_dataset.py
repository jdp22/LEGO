...
if args.simulation == 'springs':
    sim = SpringSim(noise_var=0.0, n_balls=args.n_balls,interaction_strength=args.strength)
    suffix = '_springs'
elif args.simulation == 'charged':
    sim = ChargedParticlesSim(noise_var=0.0, n_balls=args.n_balls, vel_norm=initial_vel_norm,interaction_strength=args.strength)
    suffix = '_charged'
else:
    raise ValueError('Simulation {} not implemented'.format(args.simulation))

suffix += str(args.n_balls) + "_initvel%d" % args.initial_vel + args.sufix+'_strength{}'.format(args.strength)
np.random.seed(args.seed)

print(suffix)


def generate_dataset_charge(num_sims, length, sample_freq):
    loc_all = list()
    vel_all = list()
    edges_all = list()
    charges_all = list()
...
def generate_dataset_spring(num_sims, length, sample_freq):
    loc_all = list()
    vel_all = list()
    edges_all = list()
    for i in range(num_sims):
...
if __name__ == "__main__":
    if args.simulation == 'springs':
        ...
        loc_train, vel_train, edges_train = generate_dataset_spring(args.num_train,
                                                            args.length,
                                                            args.sample_freq)

        print("Generating {} validation simulations".format(args.num_valid))
        loc_valid, vel_valid, edges_valid = generate_dataset_spring(args.num_valid,
                                                            args.length,
                                                            args.sample_freq)

        print("Generating {} test simulations".format(args.num_test))
        loc_test, vel_test, edges_test = generate_dataset_spring(args.num_test,
                                                        args.length_test,
                                                        args.sample_freq)
        ...
        print("Generating {} training simulations".format(args.num_train))
        loc_train, vel_train, edges_train, charges_train = generate_dataset_charge(args.num_train,
                                                            args.length,
                                                            args.sample_freq)

        print("Generating {} validation simulations".format(args.num_valid))
        loc_valid, vel_valid, edges_valid, charges_valid = generate_dataset_charge(args.num_valid,
                                                            args.length,
                                                            args.sample_freq)

        print("Generating {} test simulations".format(args.num_test))
        loc_test, vel_test, edges_test, charges_test = generate_dataset_charge(args.num_test,
                                                        args.length_test,
                                                        args.sample_freq)
