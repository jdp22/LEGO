import argparse
import torch
from n_body_system.dataset_nbody import NBody_SpringDynamicsDataset,NBody_Spring_Dataset
from n_body_system.model import GNN, EGNN_MoE, Baseline, Linear, EGNN_vel, Linear_dynamics, RF_vel,LLAMA_EGNN,LLAMA_EGNN_Sparse_Spring,LLAMA_EGNO_Spring
...
parser.add_argument('--test_interval', type=int, default=50, metavar='N',
...
def main():
    if args.model == 'egno' or args.model =='llama_egno':
        dataset_train = NBody_SpringDynamicsDataset(args,partition='train', dataset_name=args.dataset,max_samples=args.max_training_samples,
        ...
        dataset_val = NBody_SpringDynamicsDataset(args,partition='val',max_samples=500,
                                     dataset_name=args.dataset,num_timesteps=args.num_timesteps)
        loader_val = torch.utils.data.DataLoader(dataset_val, batch_size=args.batch_size, shuffle=False, drop_last=False,
                                             num_workers=0)
    else:
        dataset_train = NBody_Spring_Dataset(args,partition='train', dataset_name=args.dataset,
                                        max_samples=args.max_training_samples,node_number=args.node_number)
        loader_train = torch.utils.data.DataLoader(dataset_train, batch_size=args.batch_size, shuffle=True, drop_last=True)

        dataset_val = NBody_Spring_Dataset(args,partition='val', dataset_name="nbody_small",node_number=args.node_number,max_samples=args.batch_size)
        loader_val = torch.utils.data.DataLoader(dataset_val, batch_size=args.batch_size, shuffle=False, drop_last=False)
    ...
    elif args.model == 'llama_egnn':
        model = LLAMA_EGNN_Spring(in_node_nf=1, in_edge_nf=2, hidden_nf=args.nf, expert_num = 3,device=device, n_layers=args.n_layers, recurrent=True, norm_diff=args.norm_diff, tanh=args.tanh)
        model.node_number = args.node_number
        model.batch_size = args.batch_size
    elif args.model == 'llama_egno':
        model = LLAMA_EGNO_Spring(in_node_nf=1, in_edge_nf=2, hidden_nf=args.nf,expert_num = 5, device=device, n_layers=args.n_layers,
                    with_v=True, flat=False, activation=nn.SiLU(),
                    use_time_conv=False, num_modes=2, num_timesteps=args.num_timesteps)
        model.node_number = args.node_number
        model.batch_size = args.batch_size
    elif args.model == 'llama_egnn_sparse':
        model = LLAMA_EGNN_Sparse_Spring(in_node_nf=1, in_edge_nf=2, hidden_nf=args.nf, expert_num = 5,device=device, n_layers=args.n_layers, recurrent=True, norm_diff=args.norm_diff, tanh=args.tanh)
        model.node_number = args.node_number
        model.batch_size = args.batch_size
    elif args.model == 'moe':
        model = EGNN_MoE(in_node_nf=1, in_edge_nf=2, hidden_nf=args.nf, device=device, n_layers=args.n_layers, recurrent=True, norm_diff=args.norm_diff, tanh=args.tanh)
    ...
    elif args.model == 'egno':
        model = EGNO(in_node_nf=1, in_edge_nf=2, hidden_nf=args.nf, device=device, n_layers=args.n_layers,
                    with_v=True, flat=False, activation=nn.SiLU(),
                    use_time_conv=False, num_modes=2, num_timesteps=args.num_timesteps)
    ...
    for epoch in range(0, args.epochs):
        ...
        if epoch % args.test_interval == 0:
            ...
            if val_loss < best_val_loss:
                ...
                best_epoch = epoch
                torch.save(model.state_dict(), '/home/jiangdapeng/egnn/checkpoint/Spring{}_{}_model_weights.pth'.format(args.node_number,args.model))
                print("successfully saved Spring{}_{}_model".format(args.node_number,args.model))
...
def inference():
    if args.model =='llama_egno' or args.model == 'egno':
        dataset_test = NBody_SpringDynamicsDataset(args,partition='test',max_samples=500,
                                            dataset_name=args.dataset,num_timesteps=args.num_timesteps)
        loader_test = torch.utils.data.DataLoader(dataset_test, batch_size=args.batch_size, shuffle=False, drop_last=False,num_workers=0)
    else:
        dataset_test = NBody_Spring_Dataset(args,partition='test', dataset_name=args.dataset,node_number=args.node_number,max_samples=500)
    ...
    if args.model == 'gnn':
        ...
        model = LLAMA_EGNN_Sparse_Spring(in_node_nf=1, in_edge_nf=2, hidden_nf=args.nf, expert_num = 3,device=device, n_layers=args.n_layers, recurrent=True, norm_diff=args.norm_diff, tanh=args.tanh)
        model.eval()  
        model.node_number = args.node_number
        model.batch_size = args.batch_size
    elif args.model == 'llama_egno':
        ...
        raise Exception("Wrong model specified")
    model.load_state_dict(torch.load('/home/jiangdapeng/egnn/checkpoint/Spring{}_{}_model_weights.pth'.format(args.node_number,args.model)))
    optimizer = optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    best_val_loss = 1e8
    best_test_loss = 1e8
    best_epoch = 0
    test_loss = train(model, optimizer, 0 , loader_test, backprop=False,state = True)
...
def train(model, optimizer, epoch, loader, backprop=True,state = True):
    ...
    for batch_idx, data in tqdm(enumerate(loader)):
        ...
        if args.model == 'gnn':
            ...
            loc_pred = model(nodes, loc.detach(), edges, vel, edge_attr,batch_idx)
        elif args.model == 'llama_egnn_sparse':
            nodes = torch.sqrt(torch.sum(vel ** 2, dim=1)).unsqueeze(1).detach()
            rows, cols = edges
            loc_dist = torch.sum((loc[rows] - loc[cols])**2, 1).unsqueeze(1)  # relative distances among locations
            ...
            loc_pred, vel_pred, _ = model(loc.detach(), nodes, edges, edge_attr, batch_idx=batch_idx,v=vel, loc_mean=loc_mean)
        elif args.model == 'egnn_vel':
            nodes = torch.sqrt(torch.sum(vel ** 2, dim=1)).unsqueeze(1).detach()
            rows, cols = edges
            loc_dist = torch.sum((loc[rows] - loc[cols])**2, 1).unsqueeze(1)  # relative distances among locations
            ...
            loc_dist = torch.sum((loc[rows] - loc[cols])**2, 1).unsqueeze(1)  # relative distances among locations
            edge_attr = torch.cat([edge_attr, loc_dist], 1).detach()  # concatenate all edge properties
            loc_pred,balance_loss = model(nodes, loc.detach(), edges, vel, edge_attr)
        elif args.model == 'baseline':
            backprop = False
            loc_pred = model(loc)
        elif args.model == 'linear':
        ...
        if torch.isnan(loc_pred).any() or torch.isinf(loc_pred).any():
            print("Tensor contains NaN or Inf values.")
            breakpoint()
        a_loss = loss_mse(loc_pred, loc_end)
        
        if args.model == 'moe':
            loss = a_loss+balance_loss
        elif args.model == 'llama_egnn_sparse':
        ...
        if backprop:
            ...
            optimizer.step()
        if args.model == 'egno' or args.model == 'llama_egno':
            loss = loss_mse(loc_pred[-500:], loc_end[-500:])
    ...
    print("[Distance] loss x:%.6f loss y:%.6f loss z:%.6f"% (loss_mse(loc_pred[-500:,0], loc_end[-500:,0]),loss_mse(loc_pred[-500:,1], loc_end[-500:,1]),loss_mse(loc_pred[-500:,2], loc_end[-500:,2])))
